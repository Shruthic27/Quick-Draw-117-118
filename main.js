...
function s(){
n=Math.floor((Math.random() * quick_draw_data_set.length) + 1); 
std=quick_draw_data_set[n] 
...
if (results.length>0){
console.log(results)
document.getElementById('i1').innerHTML = 'Your Sketch: '+results[0].label;

document.getElementById('i2').innerHTML = 'Confidence: '+Math.round(results[0].confidence*100)+'%';

uT=new SpeechSynthesisUtterance(results[0].label);
synth.speak(uT);
}
if(results[0]==n){
    score=document.getElementById("s").value
document.getElementById("s").innerHTML =score+1
}
}